---
title: Prometheus: unexpected end of JSON input
url: https://devopstales.github.io/linux/prometheus-misbehaving-unexpected-end-of-json-input/
date: 2022-04-26
keywords: prometheus
---
...
* [Looks like the fix will be](https://github.com/prometheus/prometheus/issues/4058) to remove the dodgy directory and restart prometheus. (prometheus issue 4058)
* Looks like there’s [an issue due to be fixed in 2.9.0](https://github.com/prometheus/prometheus/issues/5457); this occurred with the bundled 2.8.1. (prometheus issue 5457, referring to 4058)
* If the file (meta.json) wasn’t empty, there’s a [python incantation](https://github.com/prometheus/prometheus/issues/4297) to validate it. (prometheus issue 4297)
* It seems to be (have been) a thing [on NFS which](https://www.bountysource.com/issues/45901178-data-corruption-using-prometheus-docker-v2-0-0-alpha-2-image-on-nfs) is not recommended.
* Also [seen on (I think) Openshift 4.1](https://bugzilla.redhat.com/show_bug.cgi?id=1664174) running Prometheus 2.3.2 on Gluster (which is probably against recommendations, like NFS)
...
[Looking at issue 4058](https://github.com/prometheus/prometheus/issues/4058), it’s noted that the code doesn’t do a sync before closing the file; it does everywhere else, and I guess this is the fix that’ll be in 2.9.0. That might have helped, though it’s a case, here, of putting on a tourniquet before shooting oneself in a foot.
