---
title: Understanding kubernetes networking: owerlay networks
url: https://devopstales.github.io/kubernetes/kubernetes-networking-2/
date: 2021-08-31
keywords: kubernetes deployment, owerlay network, CNI, Container Network Interface, calico, cilium, Flannel
---


In my previous posts We check [how the pod networking](/kubernetes-networking-1/) and [How Kubernetes use services for loadbalancing](/kubernetes-networking-1/). Now we check how the diferente Networking solutions works. How Theas solutins link the kubernetes nodes together.
<!--more-->
...
### Flannel
Flannel runs a small, single binary agent called flanneld on each host, and is responsible for allocating a subnet lease to each host out of a larger, preconfigured address space. Flannel uses either the Kubernetes API or etcd directly to store the network configuration, the allocated subnets, and any auxiliary data (such as the host's public IP). Packets are forwarded using one of several backend mechanisms including VXLAN and various cloud integrations. (Refer to [Flannel](https://github.com/flannel-io/flannel))

![Flannel](/img/include/flannel-01.webp)
...
![Calico](/img/include/calicodd.webp)
...
### Cilium
Cilium is one of the most advanced and powerful Kubernetes networking solutions. At its core, it utilizes the power of [eBPF](https://ebpf.io/) to perform a wide range of functionality ranging from traffic filtering for NetworkPolicies all the way to CNI and [kube-proxy replacement](https://k8s.networkop.co.uk/cni/cilium/). BPF is basically the ability of an application developer to write a program, load that into the Linux kernel, then run it when certain events happen. 

![Cilium](/img/include/ciliumart2.webp)
