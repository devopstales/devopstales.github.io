...
url: https://devopstales.github.io/kubernetes/vcluster/
date: 2021-08-03
keywords: kubernetes deployment, K3S, vCluster, Multi-Tenancy
---
...
We all know about `k3d` is not, it is a lightweight wrapper to run `k3s` n docker. In that scenario a Kubernetes node is a container running on docker. Probably `k3sd` was the inspiration for `vCluster` but they take the idea to the next level. With `vCluster` you can run a `k3s` cluster in a single namespace This solution is similar the [kcp](https://github.com/kcp-dev/kcp) but they used the tools that already exists.  

![vcluster - Architecture](/img/include/vcluster-architecture.svg)
